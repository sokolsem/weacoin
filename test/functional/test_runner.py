...
def main():
    ...
    tmpdir = "%s/weacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and weacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "weacoind"]) is not None:
            print("%sWARNING!%s There is already a weacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "WEACOIND" not in os.environ:
        os.environ["WEACOIND"] = build_dir + '/src/weacoind' + exeext
        os.environ["WEACOINCLI"] = build_dir + '/src/weacoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `weacoin-cli help` (`rpc_interface.txt`).
